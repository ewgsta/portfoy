...
const PaginationControls = ({ page, setPage, total }: { page: number, setPage: (n: number) => void, total: number }) => (
    <div className="flex justify-center items-center gap-4 mt-4">
        <button 
          ...
          className="p-2 rounded bg-white/5 disabled:opacity-30 hover:bg-white/10 transition-colors"
        >
            <ChevronLeft size={20} />
        </button>
...
const InputGroup = ({ label, value, onChange, type="text" }: { label: string, value: string, onChange: (v: string) => void, type?: string }) => (
    <div>
        ...
        <input 
            ...
            className="w-full bg-[#020617] border border-white/10 rounded-lg p-3 text-white focus:border-sky-500 outline-none transition-colors"
        />
    </div>
);
...
const TextAreaGroup = ({ label, value, onChange, rows=3 }: { label: string, value: string, onChange: (v: string) => void, rows?: number }) => (
    <div>
        ...
        <textarea 
            ...
            value={value}
            onChange={e => onChange(e.target.value)}
            className="w-full bg-[#020617] border border-white/10 rounded-lg p-3 text-white focus:border-sky-500 outline-none resize-none transition-colors"
        />
    </div>
);

const AdminPanel: React.FC<AdminPanelProps> = ({ 
    projects, siteConfig, messages, onUpdateProjects, onUpdateConfig, onUpdateMessages, onExit 
}) => {
  ...
  const handleSaveProject = async () => {
    ...
    // Normalize tags
    const tags = typeof projectFormData.tags === 'string' 
        ? (projectFormData.tags as string).split(',').map((t: string) => t.trim()).filter(Boolean)
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-200 flex font-sans overflow-hidden">
        ...
        <motion.aside 
            ...
            className="bg-[#0f172a] border-r border-white/5 flex flex-col z-20 shrink-0"
        >
        ...
        <main className="flex-1 flex flex-col h-screen overflow-hidden relative w-full">
            ...
            <header className="h-20 bg-[#020617]/80 backdrop-blur-md border-b border-white/5 px-8 flex items-center justify-between z-10 shrink-0">
                <button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-2 text-slate-400 hover:text-white rounded-lg hover:bg-white/5">
                    <Menu size={24} />
                </button>
                    ...
                    <div className="space-y-8 max-w-6xl mx-auto">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div className="bg-[#0f172a] p-6 rounded-2xl border border-white/5">
                                ...
                                <div className="text-3xl font-bold text-white mb-1">{messages.filter(m => !m.isRead).length}</div>
                ...
                {activeTab === 'content' && (
                    <div className="max-w-6xl mx-auto space-y-6 pb-20">
                        ...
                        <FormSection title="Hero Bölümü">
                            <div className="grid md:grid-cols-2 gap-6">
                                <InputGroup 
                                    ...
                                    onChange={v => setConfigFormData({...configFormData, hero: {...configFormData.hero, title: v}})} 
                                />
                                <InputGroup 
                                    label="Buton Metni" 
                                    value={configFormData.hero.ctaText} 
                                    onChange={v => setConfigFormData({...configFormData, hero: {...configFormData.hero, ctaText: v}})} 
                                />
                            </div>
                            <TextAreaGroup 
                                label="Alt Metin" 
                                value={configFormData.hero.subtitle} 
                                onChange={v => setConfigFormData({...configFormData, hero: {...configFormData.hero, subtitle: v}})} 
                            />
                        </FormSection>
                        ...
                        <FormSection title="Hakkımda Bölümü">
                            <InputGroup 
                                label="Başlık" 
                                value={configFormData.about.title} 
                                onChange={v => setConfigFormData({...configFormData, about: {...configFormData.about, title: v}})} 
                            />
                            <TextAreaGroup 
                                label="Detaylı Açıklama" 
                                value={configFormData.about.description} 
                                onChange={v => setConfigFormData({...configFormData, about: {...configFormData.about, description: v}})} 
                                rows={5}
                            />
                        </FormSection>
                        ...
                        <FormSection title="Projeler Bölümü Ayarları">
                            <div className="grid md:grid-cols-2 gap-6">
                                <InputGroup 
                                    label="Bölüm Başlığı" 
                                    value={configFormData.projectsSection.title} 
                                    onChange={v => setConfigFormData({...configFormData, projectsSection: {...configFormData.projectsSection, title: v}})} 
                                />
                                <InputGroup 
                                    label="GitHub Buton Metni" 
                                    value={configFormData.projectsSection.githubButtonText} 
                                    onChange={v => setConfigFormData({...configFormData, projectsSection: {...configFormData.projectsSection, githubButtonText: v}})} 
                                />
                            </div>
                            <InputGroup 
                                label="Alt Başlık" 
                                value={configFormData.projectsSection.subtitle} 
                                onChange={v => setConfigFormData({...configFormData, projectsSection: {...configFormData.projectsSection, subtitle: v}})} 
                            />
                        </FormSection>
                        ...
                        <FormSection title="İletişim & Footer">
                            <div className="grid md:grid-cols-2 gap-6">
                                <InputGroup 
                                    label="Bölüm Başlığı" 
                                    value={configFormData.contact.title} 
                                    onChange={v => setConfigFormData({...configFormData, contact: {...configFormData.contact, title: v}})} 
                                />
                                <InputGroup 
                                    label="Alt Başlık" 
                                    value={configFormData.contact.subtitle} 
                                    onChange={v => setConfigFormData({...configFormData, contact: {...configFormData.contact, subtitle: v}})} 
                                />
                                <InputGroup 
                                    label="Form Başlığı" 
                                    value={configFormData.contact.formTitle} 
                                    onChange={v => setConfigFormData({...configFormData, contact: {...configFormData.contact, formTitle: v}})} 
                                />
                                <InputGroup 
                                    label="Form Buton Metni" 
                                    value={configFormData.contact.buttonText} 
                                    onChange={v => setConfigFormData({...configFormData, contact: {...configFormData.contact, buttonText: v}})} 
                                />
                                <InputGroup 
                                    label="Email Placeholder" 
                                    value={configFormData.contact.emailPlaceholder} 
                                    onChange={v => setConfigFormData({...configFormData, contact: {...configFormData.contact, emailPlaceholder: v}})} 
                                />
                                <InputGroup 
                                    label="Mesaj Placeholder" 
                                    value={configFormData.contact.messagePlaceholder} 
                                    onChange={v => setConfigFormData({...configFormData, contact: {...configFormData.contact, messagePlaceholder: v}})} 
                                />
                            </div>
                            <div className="grid md:grid-cols-3 gap-4 pt-4 border-t border-white/5">
                                <InputGroup 
                                    label="Email Bilgisi" 
                                    value={configFormData.contact.infoEmail} 
                                    onChange={v => setConfigFormData({...configFormData, contact: {...configFormData.contact, infoEmail: v}})} 
                                />
                                <InputGroup 
                                    label="Telefon Bilgisi" 
                                    value={configFormData.contact.infoPhone} 
                                    onChange={v => setConfigFormData({...configFormData, contact: {...configFormData.contact, infoPhone: v}})} 
                                />
                                <InputGroup 
                                    label="Adres Bilgisi" 
                                    value={configFormData.contact.infoAddress} 
                                    onChange={v => setConfigFormData({...configFormData, contact: {...configFormData.contact, infoAddress: v}})} 
                                />
                            </div>
                            <div className="pt-4">
                                <InputGroup 
                                    label="Footer Metni" 
                                    value={configFormData.contact.footerText} 
                                    onChange={v => setConfigFormData({...configFormData, contact: {...configFormData.contact, footerText: v}})} 
                                />
                            </div>
                        </FormSection>
                    </div>
                ...
                {activeTab === 'projects' && (
                    <div className="max-w-6xl mx-auto h-full flex flex-col">
                        <div className="flex justify-between items-center mb-6 shrink-0">
                            ...
                            <button onClick={handleAddNewProject} className="bg-sky-600 hover:bg-sky-500 text-white px-4 py-2 rounded-lg flex items-center gap-2">
                        ...
                        <div className="bg-[#0f172a] rounded-2xl border border-white/5 overflow-hidden flex-1 flex flex-col">
                                            ...
                                            <tr key={p.id} className="hover:bg-white/5 transition-colors">
                                                ...
                                                <td className="p-4">
                                                    <div className="flex flex-wrap gap-1">
                                                        {p.tags.map(t => <span key={t} className="text-[10px] px-2 py-0.5 rounded bg-white/10 text-slate-300">{t}</span>)}
                                                    </div>
                                                </td>
                            ...
                            <div className="p-4 border-t border-white/5 mt-auto">
                                <PaginationControls page={projectsPage} setPage={setProjectsPage} total={totalPages(projects)} />
                            </div>
                        </div>
                    </div>
                )}
                ...
                {activeTab === 'messages' && (
                    <div className="max-w-4xl mx-auto h-full flex flex-col">
                        ...
                        <div className="space-y-4 flex-1 overflow-y-auto pr-2">
                            {messages.length === 0 ? (
                                <div className="text-center text-slate-500 py-20">Henüz mesaj yok.</div>
                            ) : paginate(messages, messagesPage).map((msg: ContactMessage) => (
                                <div key={msg.id} className={`bg-[#0f172a] p-6 rounded-2xl border ${msg.isRead ? 'border-white/5 opacity-70' : 'border-sky-500/30 shadow-lg shadow-sky-900/10'} transition-all`}>
                                    ...
                                    <p className="text-slate-300 text-sm mb-4 leading-relaxed bg-black/20 p-4 rounded-lg">
                                        {msg.message}
                                    </p>
                                    <div className="flex gap-3 justify-end">
                                        <button onClick={() => handleToggleRead(msg.id)} className="text-xs px-3 py-1.5 rounded border border-white/10 hover:bg-white/5 text-slate-400 transition-colors">
                                            {msg.isRead ? 'Okunmadı İşaretle' : 'Okundu İşaretle'}
                                        </button>
                                        <button onClick={() => handleDeleteMessage(msg.id)} className="text-xs px-3 py-1.5 rounded border border-red-500/20 text-red-400 hover:bg-red-500/10 transition-colors">
                                            Sil
                                        </button>
                                    </div>
                                </div>
                         ...
                         <div className="p-4 mt-auto">
                            <PaginationControls page={messagesPage} setPage={setMessagesPage} total={totalPages(messages)} />
                        </div>
                    </div>
                )}
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            <FormSection title="SEO Ayarları">
                                <InputGroup 
                                    ...
                                    value={configFormData.seo.title} 
                                    onChange={v => setConfigFormData({...configFormData, seo: {...configFormData.seo, title: v}})} 
                                />
                                <TextAreaGroup 
                                    label="Meta Açıklama (Description)" 
                                    value={configFormData.seo.description} 
                                    onChange={v => setConfigFormData({...configFormData, seo: {...configFormData.seo, description: v}})} 
                                    rows={4}
                                />
                                <InputGroup 
                                    label="Anahtar Kelimeler (Keywords)" 
                                    value={configFormData.seo.keywords} 
                                    onChange={v => setConfigFormData({...configFormData, seo: {...configFormData.seo, keywords: v}})} 
                                />
                            </FormSection>
                            ...
                            <div>
                                ...
                                <div className="bg-white p-6 rounded-2xl shadow-sm font-sans">
                                    <div className="text-[#1a0dab] text-xl cursor-pointer hover:underline truncate">
                                        {configFormData.seo.title || "Sayfa Başlığı"}
                                    </div>
                                ...
                                <p className="text-xs text-slate-500 mt-4 px-2">
                                    * Bu önizleme tahmini bir görünümdür, arama motorlarında farklılık gösterebilir.
                                </p>
                            </div>
                        </div>
                    </div>
        ...
        {/* Project Modal (Keeping Manual Save here as it is CRUD operation) */}
        <AnimatePresence>
            {isModalOpen && (
                <motion.div 
                    initial={{ opacity: 0 }}
                ...
                >
                    <motion.div 
                        className="bg-[#0f172a] border border-white/10 w-full max-w-2xl rounded-2xl p-6 shadow-2xl space-y-4"
            ...
            <AnimatePresence>
                {toasts.map(toast => (
                    <motion.div
                        ...
                        exit={{ opacity: 0, x: 20 }}
                        className={`pointer-events-auto flex items-center gap-3 px-4 py-3 rounded-lg shadow-xl border ${
                            toast.type === 'success' ? 'bg-green-900/90 border-green-500/30 text-green-200' : 'bg-red-900/90 border-red-500/30 text-red-200'
                        }`}
                    >
